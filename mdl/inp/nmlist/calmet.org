...
UP2.DAT       input     1  ! UPDAT=UP2.dat !    !END!
UP3.DAT       input     1  ! UPDAT=UP3.dat !    !END!
UP4.DAT       input     1  ! UPDAT=UP4.dat !    !END!
--------------------------------------------------------------------------------
Subgroup (c)
-----------------------------------------
...
--------------
     ...
     Starting date:    Year   (IBYR)  --    No default   ! IBYR  =  KYTSSY  !
                       Month  (IBMO)  --    No default   ! IBMO  =  KYTSSM  !
                       Day    (IBDY)  --    No default   ! IBDY  =  KYTSSD  !
     Starting time:    Hour   (IBHR)  --    No default   ! IBHR  =  KYTSSH  !
                       Second (IBSEC) --    No default   ! IBSEC =  KYTSSS  !

     Ending date:      Year   (IEYR)  --    No default   ! IEYR  =  KYTSEY  !
                       Month  (IEMO)  --    No default   ! IEMO  =  KYTSEM  !
                       Day    (IEDY)  --    No default   ! IEDY  =  KYTSED  !
     Ending time:      Hour   (IEHR)  --    No default   ! IEHR  =  KYTSEH  !
                       Second (IESEC) --    No default   ! IESEC =  KYTSES  !

      UTC time zone         (ABTZ) -- No default       ! ABTZ= UTC+0900 !
         (character*8)
...
--------------
     ...
     Map projection
     (PMAP)                     Default: UTM    ! PMAP = SBTSPMAP  !
     ...
     (FEAST)                    Default=0.0     ! FEAST  = SBTSEAST  !
     (FNORTH)                   Default=0.0     ! FNORTH = SBTSNORTH  !
     ...
     (RLAT0)                    No Default      ! RLAT0 =  SBTSRLAT  !
     (RLON0)                    No Default      ! RLON0 =  SBTSRLON  !
     ...
     (DATUM)                    Default: WGS-84    ! DATUM = SBTSDATUM !
     ...
     with X the Easting and Y the Northing coordinate
            ...
            No. X grid cells (NX)      No default     ! NX =   SBTSNX  !
            No. Y grid cells (NY)      No default     ! NY =   SBTSNY  !

     Grid spacing (DGRIDKM)            No default     ! DGRIDKM = SBTSGRID !
                                       Units: km

     Reference grid coordinate of
     SOUTHWEST corner of grid cell (1,1)

        X coordinate (XORIGKM)         No default     ! XORIGKM = SBTSORGX !
        Y coordinate (YORIGKM)         No default     ! YORIGKM = SBTSORGY !
                                       Units: km
     ...
     -------------------------
        ...
        vertical grid (ZFACE(NZ+1))    No defaults
                                       Units: m
        ! ZFACE = 0., 20., 40., 80., 150., 300., 600., 1000., 1500., 2000., 3300. !
...
--------------
    ...
    OTHER MIXING HEIGHT VARIABLES
       ...
       Minimum overland mixing height        Default:  50.   ! ZIMIN = 500. !
       (ZIMIN)                               Units: meters
...
--------------
     ...
     (One record per station --  1  records in all)
         ...
         Name   ID            X coord.   Y coord.   Time   Anem.
                               (km)       (km)      zone   Ht.(m)
       ----------------------------------------------------------
! SS1  ='TS'      152       229.941      317.559    -9    10.00  !
-------------------
      1
...
--------------
     ...
     (One record per station --  1  records in all)
         ...
         Name    ID      X coord.   Y coord.  Time zone
                           (km)       (km)    
        -----------------------------------------------
! US1  ='LL'   47922    222.959    294.554    -9  !
! US2  ='LM'   47925    222.888    322.291    -9  !
! US3  ='LR'   47952    245.711    294.640    -9  !
! US4  ='TL'   47955    245.571    322.377    -9  !
-------------------
      1
        Four character string for station name
        (MUST START IN COLUMN 9)
