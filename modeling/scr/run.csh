...
## set date variables
 ...
 set end_yr = `${exedir}/advt ${current} +1 -F ccyy`
 set end_mm = `${exedir}/advt ${current} +1 -F mm`
 set end_dd = `${exedir}/advt ${current} +1 -F dd`
 set end_jd = `${exedir}/advt ${current} +1 -j`
 @ ejd = ${end_jd} - ( ${end_yr} * 1000 )
 set end_hh = `${exedir}/advt ${current} +1 -F hh`
...
## link to run folder surface data, read and write averaged pressure to surface data
 ...
 set end_yr = `${exedir}/advt ${current} +1m -F ccyy`
 set end_mm = `${exedir}/advt ${current} +1m -F mm`
 set end_dd = `${exedir}/advt ${current} +1m -F dd`
 set end_jd = `${exedir}/advt ${current} +1m -j`
 @ ejd = ${end_jd} - ( ${stt_yr} * 1000 )
 set end_hh = `${exedir}/advt ${current} +1m -F hh`
 set end_nn = `${exedir}/advt ${current} +1m -F nn`
 set stt_date = ${current}
 set end_date = `${exedir}/advt ${current} +1m -F ccyymmddhhnn`
 @ sss = ${stt_nn} * 60
 @ ess = ${end_nn} * 60
